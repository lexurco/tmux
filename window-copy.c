...
static void	window_copy_cursor_start_of_character(struct window_mode_entry *);
...
static void
window_copy_cursor_start_of_character(struct window_mode_entry *wme)
{
	struct window_copy_mode_data	*data = wme->data;
	struct screen			*back_s = data->backing;
	struct grid_reader		 gr;
	...
	grid_reader_cursor_start_of_character(&gr);
	grid_reader_get_cursor(&gr, &px, &py);
	window_copy_update_cursor(wme, px, py, 0);
...
window_copy_cursor_up(struct window_mode_entry *wme, int scroll_only)
{
	...
	if (scroll_only || data->cy == 0) {
		...
		if (norectsel) {
			...
			window_copy_update_cursor(wme, data->cx, data->cy - 1, 0);
		if (window_copy_update_selection(wme, 1, 0)) {
			if (data->cy == screen_size_y(s) - 1)
				window_copy_redraw_lines(wme, data->cy, 1);
			else
	...
	{
		window_copy_update_cursor(wme, 0, data->cy, 0);
		if (window_copy_update_selection(wme, 1, 0))
			window_copy_redraw_lines(wme, data->cy, 1);
	}

	window_copy_cursor_start_of_character(wme);
}
...
window_copy_move_mouse(struct mouse_event *m)
{
	...
	window_copy_update_cursor(wme, x, y, 0);
}
...
window_copy_start_drag(struct client *c, struct mouse_event *m)
{
	...
	case SEL_WORD:
		...
		break;
	case SEL_LINE:
		window_copy_update_cursor(wme, 0, y, 0);
		break;
	case SEL_CHAR:
		window_copy_update_cursor(wme, x, y, 0);
		window_copy_start_selection(wme);
    ...
    u_int oy, u_int oldy, u_int px, u_int py)
{
	...
	window_copy_update_cursor(wme, px, cy, 1);
	if (window_copy_update_selection(wme, 1, 0))
    ...
    u_int sy, u_int oy, u_int oldy, u_int px, u_int py, int no_reset)
{
	...
	if (cy > yy)
		window_copy_update_cursor(wme, px, yy, 1);
	else
		window_copy_update_cursor(wme, px, cy, 1);
